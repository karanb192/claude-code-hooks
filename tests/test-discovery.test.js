 ...
 *
 * Run: node --test tests/test-discovery.test.js
 * Or:  npm test
 *
 * The `npm test` glob: "tests/*.test.js plugins/**\/tests/*.test.js" -
...
const META_SHAPE = /^tests\/[^/]+\.test\.js$/;
...
test('every *.test.js is reachable by the npm test glob', () => {
  ...
  // anywhere else (plugins/<name>/, site/, repo root, …) must
  ...
  const orphans = all.filter((p) => !META_SHAPE.test(p) && !PLUGIN_SHAPE.test(p));
