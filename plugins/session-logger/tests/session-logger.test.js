 ...
 *
 * Run: node --test plugins/session-logger/tests/session-logger.test.js
 * Or:  npm test
 */
...
// Keep ambient overrides out so the default-truncation tests see a clean env.
delete process.env.CC_SESSION_BASH_TRUNCATE;

const SCRIPT_PATH = path.join(__dirname, '../session-logger.js');
const {
  gitInfo,
  sessionFilePath,
  findExistingFile,
...
function runHook(payload, extraEnv = {}) {
  return new Promise((resolve, reject) => {
    const child = spawn('node', [SCRIPT_PATH], {
      env: { ...process.env, CC_SESSION_LOG_DIR: TMP_SESSIONS, ...extraEnv },
    });
    let stdout = '', stderr = '';
...
// ─────────────────────────────────────────────────────────────────────────────
// Env override: CC_SESSION_BASH_TRUNCATE
// ─────────────────────────────────────────────────────────────────────────────

describe('Env override: CC_SESSION_BASH_TRUNCATE', () => {
  beforeEach(cleanSessionsDir);

  it('defaults to 200 when env is unset', async () => {
    await runHook({ hook_event_name: 'SessionStart', session_id: 'trunc-def', cwd: '/tmp' });
    await runHook({
      hook_event_name: 'PostToolUse',
      session_id: 'trunc-def',
      tool_name: 'Bash',
      tool_input: { command: 'a'.repeat(250) },
    });
    const f = fs.readdirSync(TMP_SESSIONS).find(x => x.endsWith('.md'));
    const body = fs.readFileSync(path.join(TMP_SESSIONS, f), 'utf8');
    assert.ok(body.includes('a'.repeat(200) + '…'), 'should truncate at 200 chars');
    assert.ok(!body.includes('a'.repeat(201)), 'should not log past 200 chars');
  });

  it('respects a valid positive-integer override', async () => {
    const env = { CC_SESSION_BASH_TRUNCATE: '50' };
    await runHook({ hook_event_name: 'SessionStart', session_id: 'trunc-ovr', cwd: '/tmp' }, env);
    await runHook({
      hook_event_name: 'PostToolUse',
      session_id: 'trunc-ovr',
      tool_name: 'Bash',
      tool_input: { command: 'b'.repeat(120) },
    }, env);
    const f = fs.readdirSync(TMP_SESSIONS).find(x => x.endsWith('.md'));
    const body = fs.readFileSync(path.join(TMP_SESSIONS, f), 'utf8');
    assert.ok(body.includes('b'.repeat(50) + '…'), 'should truncate at 50 chars');
    assert.ok(!body.includes('b'.repeat(51)), 'should not log past 50 chars');
  });

  it('falls back to 200 on invalid values', async () => {
    const badValues = ['not-a-number', '0', '-5', '3.5'];
    for (let i = 0; i < badValues.length; i++) {
      const env = { CC_SESSION_BASH_TRUNCATE: badValues[i] };
      const sid = `bad${i}-trunc`;
      await runHook({ hook_event_name: 'SessionStart', session_id: sid, cwd: '/tmp' }, env);
      await runHook({
        hook_event_name: 'PostToolUse',
        session_id: sid,
        tool_name: 'Bash',
        tool_input: { command: 'c'.repeat(250) },
      }, env);
      const f = fs.readdirSync(TMP_SESSIONS).find(x => x.endsWith(`_bad${i}-tru.md`));
      const body = fs.readFileSync(path.join(TMP_SESSIONS, f), 'utf8');
      assert.ok(body.includes('c'.repeat(200) + '…'), `"${badValues[i]}" should fall back to 200`);
      assert.ok(!body.includes('c'.repeat(201)), `"${badValues[i]}" should not log past 200 chars`);
    }
  });
});
