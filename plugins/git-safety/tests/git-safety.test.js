...
// The unit tests below assume the default safety level; clear any ambient
// override before the module is loaded. The spawn-based env-override tests
// set HOOK_SAFETY_LEVEL explicitly per child process.
delete process.env.HOOK_SAFETY_LEVEL;

const { PATTERNS, PROTECTED_BRANCHES, LEVELS, checkCommand } = require('../git-safety.js');

const SCRIPT_PATH = path.join(__dirname, '../git-safety.js');
...
// ─────────────────────────────────────────────────────────────────────────────
// Integration Tests - HOOK_SAFETY_LEVEL env override
// ─────────────────────────────────────────────────────────────────────────────

// Spawn the hook with HOOK_SAFETY_LEVEL controlled explicitly (unset when
// `level` is undefined) and cwd pointed at a non-git temp dir, so branch-aware
// patterns cannot fire from whatever branch the test runner happens to be on.
function runHookWithLevel(command, level, cwd) {
  return new Promise((resolve, reject) => {
    const env = { ...process.env };
    delete env.HOOK_SAFETY_LEVEL;
    if (level !== undefined) env.HOOK_SAFETY_LEVEL = level;

    const child = spawn('node', [SCRIPT_PATH], { cwd, env });
    let stdout = '';
    let stderr = '';
...
describe('Integration: HOOK_SAFETY_LEVEL env override', () => {
  let nonGitCwd;

  before(() => {
    nonGitCwd = fs.mkdtempSync(path.join(os.tmpdir(), 'git-safety-env-'));
  });

  after(() => {
    try { fs.rmSync(nonGitCwd, { recursive: true, force: true }); } catch {}
  });

  it('env unset → default high: plain force-push allowed (delegated to sibling)', async () => {
    const { code, output } = await runHookWithLevel('git push --force origin feature', undefined, nonGitCwd);
    assert.strictEqual(code, 0);
    assert.deepStrictEqual(output, {});
  });

  it('env unset → default high: gh pr merge still denied', async () => {
    const { output } = await runHookWithLevel('gh pr merge 1', undefined, nonGitCwd);
    assert.strictEqual(output.hookSpecificOutput?.permissionDecision, 'deny');
    assert.ok(output.hookSpecificOutput?.permissionDecisionReason.includes('gh-pr-merge'));
  });

  it('HOOK_SAFETY_LEVEL=strict → plain force-push denied', async () => {
    const { output } = await runHookWithLevel('git push --force origin feature', 'strict', nonGitCwd);
    assert.strictEqual(output.hookSpecificOutput?.permissionDecision, 'deny');
    assert.ok(output.hookSpecificOutput?.permissionDecisionReason.includes('force-push'));
  });

  it('HOOK_SAFETY_LEVEL=critical → gh pr merge allowed (no rules apply)', async () => {
    const { code, output } = await runHookWithLevel('gh pr merge 1', 'critical', nonGitCwd);
    assert.strictEqual(code, 0);
    assert.deepStrictEqual(output, {});
  });

  it('invalid HOOK_SAFETY_LEVEL falls back to high: force-push allowed', async () => {
    const { output } = await runHookWithLevel('git push --force origin feature', 'bogus', nonGitCwd);
    assert.deepStrictEqual(output, {});
  });

  it('invalid HOOK_SAFETY_LEVEL falls back to high: gh pr merge denied', async () => {
    const { output } = await runHookWithLevel('gh pr merge 1', 'bogus', nonGitCwd);
    assert.strictEqual(output.hookSpecificOutput?.permissionDecision, 'deny');
    assert.ok(output.hookSpecificOutput?.permissionDecisionReason.includes('gh-pr-merge'));
  });
});
